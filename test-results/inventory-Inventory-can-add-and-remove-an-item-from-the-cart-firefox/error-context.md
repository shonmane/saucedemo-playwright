...
- Name: inventory.spec.ts >> Inventory >> can add and remove an item from the cart
- Location: tests\inventory.spec.ts:13:7
...
> 25 |     await this.loginButton.click();
  ...
  26 |   }
  27 | 
  28 |   async expectErrorMessage(text: string) {
  29 |     await expect(this.errorMessage).toBeVisible();
  30 |     await expect(this.errorMessage).toContainText(text);
  31 |   }
  32 | }
  33 | 
```